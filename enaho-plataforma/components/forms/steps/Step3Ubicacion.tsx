...
export default function Step3Ubicacion({ data, update }: Props) {
  ...
  return (
    <div>
      <OptionGrid label="Región natural" cols={3}>
        {(['costa','sierra','selva'] as Region[]).map(v => (
          <OptionItem key={v} label={v.charAt(0).toUpperCase() + v.slice(1)}
            selected={data.region === v} onClick={() => update({ region: v })} />
        ))}
      </OptionGrid>
